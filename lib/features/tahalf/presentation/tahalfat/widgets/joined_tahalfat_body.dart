...
class JoinedTahalfatBody extends StatelessWidget {
  const JoinedTahalfatBody({super.key});
          ...
          child: Column(
            children: [
              ...
              if (state.status == TahalfatStatus.loaded &&
                  state.joinedTahalfList.isEmpty)
                const NoTahalfat(),
              ListView.builder(
                shrinkWrap: true,
                physics: const NeverScrollableScrollPhysics(),
                itemCount: state.joinedTahalfList.length,
                itemBuilder: (context, index) {
                  final tahalf = state.joinedTahalfList[index];
                  final int limit;
          ...
          itemBuilder: (context) {
            return [
              cardPopupMenuItem(
                ...
                context: context,
                menuItemTitle: 'بلاغ ',
                dialogTitle: 'بلاغ عن تحالف ',
                buttonText: 'ارسال',
                dialogBody: WassetTextField(
                  maxLines: 3,
                  hintText: 'اكتب السبب هنا ',
                  onChanged: (description) {
                    context.read<TahalfatCubit>().setDescription(description);
                  },
                ),
                dailogButtonFunc: () {
                  context
              ...
              cardPopupMenuItem(
                ...
                context: context,
                menuItemTitle: 'خروج ',
                dialogTitle: 'هل أنت متأكد من انك تريد الخروج من التحالف ؟ ',
                buttonText: 'خروج',
                dailogButtonFunc: () {
                  context.pop();
                  context.read<TahalfatCubit>().exitFromTahalf(tahalf.id!);
                },
              ),
            ];
          },
